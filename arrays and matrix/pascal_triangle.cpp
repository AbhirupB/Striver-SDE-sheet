...
Use the combination formula: nCr = n!/(n-r)! * (r)!
using the formula directly makes TC:O(n)+O(r)+O(n-r)
...
// TC:O(c)  We are running a loop for r times, where r is c-1.
// SC:O(1)  
...
//TC: O(n*n*r) ~ O(n^3), where n = number of rows, and r = column index.
